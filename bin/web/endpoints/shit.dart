...
class ShitWebsocket extends Websocket {
  @override
  AuthMethod authMethod = AuthMethod.Query;

  ShitWebsocket([String route = '/shit']) : super(route: route);
...
//    final maxBuffer = 10001000; // 10.001MB (decimal to prevent de-syncing between this and core)
    ...
    client
        .sendFile(streamController.stream)
        .printErrors()
        .listen((response) {
    ...
    uploadRequest.listen((response) {
      if (response.uploadStatus == UploadResponse_UploadStatus.READY) {
        completer.complete();
      } else if (response.uploadStatus == UploadResponse_UploadStatus.COMPLETE) {
